...
class Point {
  ...
  //Write a method allWithin that takes an array of Points and an integer
  //dist and returns a new array of all points within dist of this point
  //from that array
  Point[] allWithin(Point[] pts, int dist) {
    ...
    for (Point p: pts) {
      if (this.distance(p) < dist) {
        toReturn[index] = p;    //array update expressions
        index += 1;
      }
    }
